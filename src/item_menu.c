...
static void BagListMenuItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != -2 && sBagMenuDisplay->nItems[gBagMenuState.pocket] != itemId)
    {
        ...
        if (((gBagMenuState.pocket != POCKET_KEY_ITEMS - 1 ) && ItemId_GetImportance(bagItemId) == 0) || bagItemId == ITEM_ODD_CANDY || bagItemId == ITEM_BRICK_PIECE || bagItemId == ITEM_NOTHING)
        {
