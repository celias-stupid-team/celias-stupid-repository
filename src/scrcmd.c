...
bool8 ScrCmd_buffermovename(struct ScriptContext * ctx)
{
    ...
    StringCopy(sScriptStringVars[stringVarIndex], gLongMoveNames[moveId]);
    return FALSE;
}
...
bool8 ScrCmd_checkfieldmove(struct ScriptContext * ctx)
{
    ...
    if(moveId == MOVE_SURF && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_ROUTE12)) {
        gSpecialVar_Result = PARTY_SIZE;
        return FALSE;
    }
