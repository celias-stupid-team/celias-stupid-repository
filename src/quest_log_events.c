...
static const u8 *const sLocationNameTexts[] =
{
    ...
    [QL_LOCATION_FUSHCIA_GYM]        = gText_QuestLog_Gym,
...
static const u8 sLocationToDepartedTextId[] =
{
    ...
    [QL_LOCATION_FUSHCIA_GYM]        = QL_DEPARTED_GYM,
...
static const u16 *LoadEvent_UsedItem(const u16 *eventData)
{
    ...
    case POCKET_TM_CASE:
        ...
        StringCopy(gStringVar2, gLongMoveNames[ItemIdToBattleMoveId(rItemId)]);
        if (rItemParam != 0xFFFF)
        {
            StringCopy(gStringVar3, gLongMoveNames[rItemParam]);
