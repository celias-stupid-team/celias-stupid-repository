...
#define DEFENDER_IS_PROTECTED (((gProtectStructs[gBattlerTarget].protected || gProtectStructs[gBattlerTarget].banefulBunker || gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)] & SIDE_STATUS_SPIKY_SHIELD) && (gBattleMoves[gCurrentMove].flags & FLAG_PROTECT_AFFECTED)) || (gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)] & SIDE_STATUS_SHADOW_SHIELD && (!(gBattleTypeFlags & BATTLE_TYPE_ZAPMOLCUNOOHGIA) || gCurrentMove != MOVE_RAINBOW_BEAM)))
...
static const u16 sNaturePowerMoves[] =
{
    ...
    [BATTLE_TERRAIN_RAINBOW]    = MOVE_RAINBOW_BEAM,
...
static const u8 sTerrainToType[] =
{
    ...
    [BATTLE_TERRAIN_RAINBOW]    = TYPE_FAIRY,
};
...
static void Cmd_attackcanceler(void)
{
    ...
    if (gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)] & SIDE_STATUS_SHADOW_SHIELD && (!(gBattleTypeFlags & BATTLE_TYPE_ZAPMOLCUNOOHGIA) || gCurrentMove != MOVE_RAINBOW_BEAM))
    {
...
static void Cmd_critcalc(void)
{
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_TORRENT) //Starter can't be crit
...
static void CheckWonderGuardAndLevitate(void)
{
    ...
    if (mult >= TYPE_MUL_SUPER_EFFECTIVE)
        flags |= 1;
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        else // Shadow Sky weather prevents primary status conditions
        {
            BattleScriptPush(gBattlescriptCurrInstr + 1);
    ...
    else // secondary effects
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_CONFUSION:
                ...
                else if (gBattleMons[gEffectBattler].ability == ABILITY_OWN_TEMPO
...
static void Cmd_various(void)
{
    ...
    switch (gBattlescriptCurrInstr[2])
    {
        ...
        case VARIOUS_JUMP_IF_NO_HOLD_EFFECT:
        {
            ...
            if (ItemId_GetHoldEffect(gBattleMons[gActiveBattler].item) != cmd->holdEffect)
            {
...
static void Cmd_weatherdamage(void)
{
    ...
    if (WEATHER_HAS_EFFECT || (gBattleWeather & B_WEATHER_SHADOW_SKY))
    {
        ...
        if (gBattleWeather & B_WEATHER_SHADOW_SKY)
        {
            if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_SHADOW) && !gSpeciesInfo[gBattleMons[gBattlerAttacker].species].isShadowType)
            {
...
static void Cmd_sethail(void)
{
    if (gBattleWeather & B_WEATHER_HAIL || gBattleWeather & B_WEATHER_SHADOW_SKY || gBattleWeather & B_WEATHER_GRAVITY)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_FAILED;
    }
...
static void Cmd_handleballthrow(void)
{
    ...
    else if (gBattleTypeFlags & BATTLE_TYPE_KANGA)
    {
        BtlController_EmitBallThrowAnim(BUFFER_A, BALL_TRAINER_BLOCK);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr = BattleScript_CantCatchNoSnagMachine;
    }
    ...
    else
    {
        u32 odds;
        u8 catchRate;
        // SDH: The ball the player chose from the bag.
        // Also gets set to the target mon's "MON_DATA_POKEBALL" data if the catch was successful.
        u16 thrownBall = ItemId_GetSecondaryId(gLastUsedItem);
        ...
        if (thrownBall != SAFARI_BALL)
        {
            ...
            {
                if (gBattleResults.catchAttempts[thrownBall - ULTRA_BALL] < 255)
...
void BS_JumpIfZapmolcunoSpecies(void)
{
