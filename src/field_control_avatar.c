...
static const u8 *GetInteractedMetatileScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    if (MetatileBehavior_IsDaisy(metatileBehavior) == TRUE)
        return EventScript_DaisyTile;   
    if (MetatileBehavior_IsShitWall(metatileBehavior) == TRUE)
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, u8 direction)
{
    ...
    if (FlagGet(FLAG_SYS_IS_FULL_RELEASE) == TRUE && PartyHasMonWithSurf() == TRUE && IsPlayerFacingSurfableFishableWater() == TRUE)
...
static bool8 TryStartStepBasedScript(struct MapPosition *position, u16 metatileBehavior, u16 direction)
{
    ...
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior) && UpdatePsyduckPailCounter() == TRUE)
        return TRUE;
    if (!(gPlayerAvatar.flags & PLAYER_AVATAR_FLAG_FORCED) && !MetatileBehavior_IsForcedMovementTile(metatileBehavior) && UpdateGlitchCityCounter() == TRUE)
        return TRUE;
    return FALSE;
}
