...
static void AdvanceComfyAnim_Easing(struct ComfyAnim *anim)
{
    ...
    t = Q_24_8_div(Q_24_8(anim->state.easingState.curFrame), Q_24_8(anim->config.data.easing.durationFrames));
    ...
    anim->position = config->from + Q_24_8_mul(x, (config->to - config->from));
...
static void AdvanceComfyAnim_Spring(struct ComfyAnim *anim)
{
    ...
    springForce = -1 * Q_24_8_mul(Q_24_8_mul(config->tension, 0x5 /* Q_24_8(0.02) */), anim->position - config->to);
    dampingForce = -1 * Q_24_8_mul(Q_24_8_mul(config->friction, 0x5 /* Q_24_8(0.02) */), anim->velocity);
    acceleration = Q_24_8_div(springForce + dampingForce, config->mass);
...
{
    return Q_24_8_mul(t, t);
}

s32 ComfyAnimEasing_EaseOutQuad(s32 t /* Q_24_8 */)
{
    s32 v = Q_24_8(1) - t;
    return Q_24_8(1) - Q_24_8_mul(v, v);
}

s32 ComfyAnimEasing_EaseInOutQuad(s32 t /* Q_24_8 */)
{
    if (t < (Q_24_8(1) >> 1))
        return 2 * Q_24_8_mul(t, t);
    else
    {
        s32 v = -2 * t + Q_24_8(2);
        return Q_24_8(1) - (Q_24_8_mul(v, v) >> 1);
    }
}

s32 ComfyAnimEasing_EaseInCubic(s32 t /* Q_24_8 */)
{
    return Q_24_8_mul(t, Q_24_8_mul(t, t));
}

s32 ComfyAnimEasing_EaseOutCubic(s32 t /* Q_24_8 */)
{
    s32 v = Q_24_8(1) - t;
    return Q_24_8(1) - Q_24_8_mul(v, Q_24_8_mul(v, v));
}

s32 ComfyAnimEasing_EaseInOutCubic(s32 t /* Q_24_8 */)
{
    if (t < (Q_24_8(1) >> 1))
        return 4 * Q_24_8_mul(t, Q_24_8_mul(t, t));
    else
    {
        s32 v = -2 * t + Q_24_8(2);
        return Q_24_8(1) - (Q_24_8_mul(v, Q_24_8_mul(v, v)) >> 1);
    }
}
...
s32 ComfyAnimEasing_EaseInOutBack(s32 t /* Q_24_8 */)
{
    ...
    {
        s32 v1 = 2 * t;
        s32 a = Q_24_8_mul(v1, v1);
        s32 b = Q_24_8_mul(2 * (c2 + Q_24_8(1)), t) - c2;
        return (Q_24_8_mul(a, b)) >> 1;
    }
    else
    {
        s32 v1 = 2 * t - Q_24_8(2);
        s32 a = Q_24_8_mul(v1, v1);
        s32 b = Q_24_8_mul(c2 + Q_24_8(1), v1) + c2;
        return (Q_24_8_mul(a, b) + Q_24_8(2)) >> 1;
    }
}
