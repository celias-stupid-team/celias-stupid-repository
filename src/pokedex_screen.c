...
static u32 OwnedUnobtainableMonCount(void);
...
const u16 sExtraTextPalette[] = INCBIN_U16("graphics/pokedex/kanto_dex_extra_text.gbapal");
...
void DexScreen_LoadResources(void)
{
    ...
    *sPokedexScreenData = sDexScreenDataInitialState;
    ...
    LoadPalette(sExtraTextPalette, BG_PLTT_ID(13), sizeof(sExtraTextPalette));
...
static void DexScreen_InitGfxForTopMenu(void)
{
    ...
    else
    {
        ...
        //Should be:
        // [Num obtainable && Owned] "{blue} + [Num not obtainable && Owned] / [Num Obtainable]"
        ...
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->dexCountsWindowId, FONT_NORMAL_COPY_1, gText_Owned, 0, 37, 0); //owned
...
static u16 DexScreen_CountMonsInOrderedList(u8 orderIdx)
{
    ...
    u16 max_entries;
    u16 max_entries_national;
    u8 missingEntries = 1; // to account for the missing slot for Pikachu

    if (!VictiniSeen)
        missingEntries++; // to account for the missing slot for Victini

    max_entries = KANTO_DEX_COUNT - missingEntries;
    max_entries_national = NATIONAL_DEX_COUNT - missingEntries;
    ...
    case DEX_ORDER_NUMERICAL_KANTO:
        for (i = 0; i < max_entries; i++)
        {
            ...
            obtainable = DexScreen_GetSetPokedexFlag(ndex_num, FLAG_GET_OBTAINABLE, FALSE);
            
            if (seen || caught)
            {
                if (ndex_num == NATIONAL_DEX_CASTFORM && !FlagGet(FLAG_FOUGHT_CASTFORM))
                    sPokedexScreenData->listItems[i].label = gSpeciesNames[NationalPokedexNumToSpecies(NATIONAL_DEX_MEW)];
                else
        ...
        ret++; // add 1 because we're returning the value for i which starts at 0
        break;
    case DEX_ORDER_ATOZ:
    ...
    case DEX_ORDER_LIGHTEST:
        for (i = 0; i < max_entries_national; i++)
        {
...
static void ItemPrintFunc_OrderedListMenu(u8 windowId, u32 itemId, u8 y)
{
    ...
    bool8 shinyFound = DexScreen_GetSetPokedexFlag(species, FLAG_GET_SHINY_FOUND, TRUE);
    ...
    if (caught) // Print ball and types if it's caught; otherwise nothing
    {
        if (obtainable) {
            if (shinyFound) {
                BlitMenuInfoIcon(sPokedexScreenData->numericalOrderWindowId, MENU_INFO_ICON_PREMIER, 0x28, y); // Icon Premier
            } else {
                BlitMenuInfoIcon(sPokedexScreenData->numericalOrderWindowId, MENU_INFO_ICON_CAUGHT, 0x28, y); // Icon Caught
            }
        } else {
            if (shinyFound) {
                BlitMenuInfoIcon(sPokedexScreenData->numericalOrderWindowId, MENU_INFO_ICON_THREE_STAR, 0x28, y); // Three Stars
            } else {
...
static void DexScreen_PrintObtainable(u8 windowId, u8 fontId, u16 num, u8 x, u8 y, u8 colorIdx)
{
    ...
    overflow = OwnedUnobtainableMonCount();

    if (overflow > 0)
    {
        StringCopy(gStringVar1, gText_Plus);
        ...
        overflowOffset = GetStringWidth(fontId, gStringVar2, 0);
        StringAppend(gStringVar1, gStringVar2);
        DexScreen_AddTextPrinterParameterized(windowId, fontId, gStringVar1, x - 8, y, DEX_TEXT_BLUE);
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->dexCountsWindowId, FONT_SMALL, gText_Slash, x + 1 + overflowOffset, 49, colorIdx);
        ConvertIntToDecimalStringN(gStringVar1, num, STR_CONV_MODE_LEFT_ALIGN, digits);
        DexScreen_AddTextPrinterParameterized(windowId, fontId, gStringVar1, x + 8 + overflowOffset , y, colorIdx);
    }
    else
    {
        DexScreen_AddTextPrinterParameterized(sPokedexScreenData->dexCountsWindowId, FONT_SMALL, gText_Slash, 20, 49, colorIdx);
        ConvertIntToDecimalStringN(gStringVar1, num, STR_CONV_MODE_LEFT_ALIGN, digits);
...
static u32 OwnedUnobtainableMonCount(void)
{
    ...
    {
        if (GetSetPokedexFlag(i + 1, FLAG_GET_CAUGHT) && !GetSetPokedexFlag(i + 1, FLAG_GET_OBTAINABLE))
            count++;
    }
    return count;
}
...
s8 DexScreen_GetSetPokedexFlag(u16 nationalDexNo, u8 caseId, bool8 indexIsSpecies)
{
    ...
    if (nationalDexNo > NATIONAL_DEX_COUNT)
        return 0;
    ...
    case FLAG_GET_OBTAINABLE:
        if (gSaveBlock2Ptr->pokedex.obtainable[index] & mask)
            retVal = 1;
        break;
    case FLAG_GET_SHINY_FOUND:
        if (gSaveBlock2Ptr->shinyFound[index] & mask)
            retVal = 1;
        break;
    case FLAG_SET_SEEN:
